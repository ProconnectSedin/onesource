...
BEGIN
    ...
    FROM stg.Stg_sdin_payment_dtl s
...
/*
	...
	USING stg.stg_sdin_payment_dtl s
	WHERE t.tran_ou = s.tran_ou
    AND t.tran_type = s.tran_type
    AND t.tran_no = s.tran_no
    AND t.term_no = s.term_no;
*/
    ...
    SELECT
        s.tran_type			, s.tran_ou		, s.tran_no		, s.term_no			, s.due_date,
		s.due_amount_type	, s.due_percent	, s.due_amount	, s.disc_comp_amount, s.disc_amount_type, 
		s.disc_date			, s.disc_percent, s.disc_amount	, s.penalty_percent	, s.base_due_amount,
		s.base_disc_amount	, s.guid		,
				1			, p_etljobname	, p_envsourcecd	, p_datasourcecd	, NOW()
    FROM stg.Stg_sdin_payment_dtl s
    LEFT JOIN dwh.F_sdinpaymentdtl t
    ...
    INSERT INTO raw.raw_sdin_payment_dtl
    (
