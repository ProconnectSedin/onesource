-- PROCEDURE: dwh.usp_f_dispatchdetailweekly(character varying, character varying, character varying, character varying)

-- DROP PROCEDURE IF EXISTS dwh.usp_f_dispatchdetailweekly(character varying, character varying, character varying, character varying);

CREATE OR REPLACE PROCEDURE dwh.usp_f_dispatchdetailweekly(
...
BEGIN
	...
	UPDATE dwh.f_dispatchdetail t1
	 SET etlactiveind =  0,
     etlupdatedatetime = Now()::timestamp
     FROM dwh.f_dispatchdetail t
	left join  stg.stg_wms_dispatch_dtl s
	    ON  s.wms_dispatch_loc_code 	= t.dispatch_loc_code
		AND s.wms_dispatch_ld_sheet_no 	= t.dispatch_ld_sheet_no
		AND s.wms_dispatch_ld_sheet_ou 	= t.dispatch_ld_sheet_ou
		AND s.wms_dispatch_lineno 		= t.dispatch_lineno
	where t.dispatch_dtl_key= t1.dispatch_dtl_key
	AND   COALESCE(t.etlupdatedatetime,t.etlcreatedatetime)::date >= NOW()::DATE
	and 	s.wms_dispatch_loc_code IS NULL;
...
END;
$BODY$;
ALTER PROCEDURE dwh.usp_f_dispatchdetailweekly(character varying, character varying, character varying, character varying)
    OWNER TO proconnect;
