-- PROCEDURE: dwh.usp_f_brconsignmentthuserialdetailweekly(character varying, character varying, character varying, character varying)

-- DROP PROCEDURE IF EXISTS dwh.usp_f_brconsignmentthuserialdetailweekly(character varying, character varying, character varying, character varying);

CREATE OR REPLACE PROCEDURE dwh.usp_f_brconsignmentthuserialdetailweekly(
...
BEGIN
	...
	UPDATE dwh.f_bRConsignmentTHUSerialDetail t1
	set etlactiveind        =  0,
         etlupdatedatetime   = Now()::TIMESTAMP
	from dwh.f_bRConsignmentTHUSerialDetail t
	left join stg.stg_tms_brctd_consgt_thu_serial_details s
	ON s.ctsd_ouinstance			=	t.ctsd_ouinstance
    AND s.ctsd_br_id				=	t.ctsd_br_id
    AND s.ctsd_thu_line_no			=	t.ctsd_thu_line_no
    AND s.ctsd_thu_serial_line_no	=	t.ctsd_thu_serial_line_no
	where t.ctsd_thu_key			=   t1.ctsd_thu_key
	AND   COALESCE(t.etlupdatedatetime,t.etlcreatedatetime)::date >= NOW()::DATE
	AND   s.ctsd_ouinstance IS NULL;
	
	--select * from dwh.f_bRConsignmentTHUSerialDetail limit 10;
...
END;
$BODY$;
ALTER PROCEDURE dwh.usp_f_brconsignmentthuserialdetailweekly(character varying, character varying, character varying, character varying)
    OWNER TO proconnect;
