-- PROCEDURE: dwh.usp_f_spyprepayvchhdrweekly(character varying, character varying, character varying, character varying)

-- DROP PROCEDURE IF EXISTS dwh.usp_f_spyprepayvchhdrweekly(character varying, character varying, character varying, character varying);

CREATE OR REPLACE PROCEDURE dwh.usp_f_spyprepayvchhdrweekly(
...
--Updating etlactiveind for Deleted source data 
		...
		UPDATE	dwh.F_spyprepayvchhdr t1
		SET		etlactiveind		=  0,
				etlupdatedatetime	= Now()::TIMESTAMP
		FROM	dwh.F_spyprepayvchhdr t
		LEFT JOIN stg.stg_spy_prepay_vch_hdr s
    	ON		s.ou_id				= t.ou_id
    	AND		s.voucher_no		= t.voucher_no
    	AND		s.tran_type			= t.tran_type
    	AND		s.ptimestamp		= t.ptimestamp
    	AND		s.lgt_invoice_flag	= t.lgt_invoice_flag
		WHERE	t.prepay_vch_hdr_key	= t1.prepay_vch_hdr_key
...
END;
$BODY$;
ALTER PROCEDURE dwh.usp_f_spyprepayvchhdrweekly(character varying, character varying, character varying, character varying)
    OWNER TO proconnect;
