-- PROCEDURE: dwh.usp_f_tariffrevcostdetailweekly(character varying, character varying, character varying, character varying)

-- DROP PROCEDURE IF EXISTS dwh.usp_f_tariffrevcostdetailweekly(character varying, character varying, character varying, character varying);

CREATE OR REPLACE PROCEDURE dwh.usp_f_tariffrevcostdetailweekly(
...
UPDATE dwh.F_TariffRevCostDetail t
	...
	UPDATE dwh.F_TariffRevCostDetail t1
	  SET     etlactiveind        =  0,
     		 etlupdatedatetime   = Now()::TIMESTAMP
	from 	dwh.F_TariffRevCostDetail t
	left join stg.stg_tms_tarcd_tariff_rev_cost_dtl s
	ON		s.tarcd_ouinstance			=	t.tarcd_ouinstance
    AND		s.tarcd_trip_plan_id		=	t.tarcd_trip_plan_id
    AND		s.tarcd_booking_request		=	t.tarcd_booking_request
	AND		s.tarcd_stage_of_derivation =	t.tarcd_stage_of_derivation
	AND		s.tarcd_buy_sell_type       =	t.tarcd_buy_sell_type
    AND		s.tarcd_unique_id			=	t.tarcd_unique_id
	where 	t.tarcd_dtl_key				=	t1.tarcd_dtl_key
	AND     COALESCE(t.etlupdatedatetime,t.etlcreatedatetime)::date >= NOW()::DATE
	and 	s.tarcd_ouinstance is null;
...
END;
$BODY$;
ALTER PROCEDURE dwh.usp_f_tariffrevcostdetailweekly(character varying, character varying, character varying, character varying)
    OWNER TO proconnect;
