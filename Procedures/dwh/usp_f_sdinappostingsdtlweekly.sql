-- PROCEDURE: dwh.usp_f_sdinappostingsdtlweekly(character varying, character varying, character varying, character varying)

-- DROP PROCEDURE IF EXISTS dwh.usp_f_sdinappostingsdtlweekly(character varying, character varying, character varying, character varying);

CREATE OR REPLACE PROCEDURE dwh.usp_f_sdinappostingsdtlweekly(
...
BEGIN
    ...
    UPDATE   dwh.f_sdinappostingsdtl t1
        SET     etlactiveind        =  0,
                etlupdatedatetime   = Now()::TIMESTAMP
        FROM    dwh.f_sdinappostingsdtl t
        LEFT JOIN stg.stg_sin_ap_postings_dtl s
        ON  t.tran_type = s.tran_type 
        AND  t.tran_ou = s.tran_ou 
        AND  t.tran_no = s.tran_no 
        AND  t.posting_line_no = s.posting_line_no 
        WHERE   t.f_sdinappostingsdtl_key     = t1.f_sdinappostingsdtl_key
        AND     COALESCE(t.etlupdatedatetime,t.etlcreatedatetime)::date >= NOW()::DATE
        AND     s.tran_type IS NULL;
...
END;
$BODY$;
ALTER PROCEDURE dwh.usp_f_sdinappostingsdtlweekly(character varying, character varying, character varying, character varying)
    OWNER TO proconnect;
