-- PROCEDURE: dwh.usp_f_stncreditnotedtl(character varying, character varying, character varying, character varying)

-- DROP PROCEDURE IF EXISTS dwh.usp_f_stncreditnotedtl(character varying, character varying, character varying, character varying);

CREATE OR REPLACE PROCEDURE dwh.usp_f_stncreditnotedtl(
...
BEGIN
    ...
    FROM stg.stg_stn_credit_note_dtl;

    UPDATE dwh.f_stncreditnotedtl t
    SET
        stncreditnotedtl_curr_key	 = COALESCE(cu.curr_key,-1), 
		stncreditnotedtl_vendor_key	 = COALESCE(v.vendor_key,-1),
		stncreditnotedtl_opcoa_key	 = COALESCE(ac.opcoa_key,-1),
    ...
    FROM stg.stg_stn_credit_note_dtl s
    ...
    INSERT INTO dwh.f_stncreditnotedtl
    (
		stncreditnotedtl_curr_key,	stncreditnotedtl_vendor_key,	stncreditnotedtl_opcoa_key,
        ou_id, 			trns_credit_note, 	tran_type, 			timestamp, 			transfer_docno, 
		notype_no, 		tran_date, 			supplier_code, 		currency_code, 		account_code, 
    ...
    SELECT
        ...
        s.ou_id, 		s.trns_credit_note, s.tran_type, 		s.timestamp, 		s.transfer_docno, 
		s.notype_no, 	s.tran_date, 		s.supplier_code, 	s.currency_code, 	s.account_code, 
	...
	LEFT JOIN dwh.d_currency cu     
    ON 	  s.currency_code  		= cu.iso_curr_code
    LEFT JOIN dwh.f_stncreditnotedtl t
    ON 	  s.ou_id 				= t.ou_id
    ...
    INSERT INTO raw.raw_stn_credit_note_dtl
    (
        ou_id, 			trns_credit_note, 	tran_type, 			timestamp, 			transfer_docno, 
		notype_no, 		tran_date, 			supplier_code, 		currency_code, 		account_code, 
...
END;
$BODY$;
ALTER PROCEDURE dwh.usp_f_stncreditnotedtl(character varying, character varying, character varying, character varying)
    OWNER TO proconnect;
