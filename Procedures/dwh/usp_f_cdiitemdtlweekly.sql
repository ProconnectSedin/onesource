-- PROCEDURE: dwh.usp_f_cdiitemdtlweekly(character varying, character varying, character varying, character varying)

-- DROP PROCEDURE IF EXISTS dwh.usp_f_cdiitemdtlweekly(character varying, character varying, character varying, character varying);

CREATE OR REPLACE PROCEDURE dwh.usp_f_cdiitemdtlweekly(
...
BEGIN
    ...
    --Updating etlactiveind for Deleted source data 
        ...
        UPDATE  dwh.F_cdiitemdtl t1
        SET     etlactiveind        =  0,
                etlupdatedatetime   = Now()::TIMESTAMP
        FROM    dwh.F_cdiitemdtl t
        LEFT JOIN stg.stg_cdi_item_dtl s
        ON t.tran_type              = s.tran_type
        AND t.tran_ou                   = s.tran_ou
        AND t.tran_no                   = s.tran_no
        AND t.line_no                   = s.line_no
        WHERE   t.cdi_itm_dtl_key       = t1.cdi_itm_dtl_key
...
END;
$BODY$;
ALTER PROCEDURE dwh.usp_f_cdiitemdtlweekly(character varying, character varying, character varying, character varying)
    OWNER TO proconnect;
