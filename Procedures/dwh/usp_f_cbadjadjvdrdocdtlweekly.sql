-- PROCEDURE: dwh.usp_f_cbadjadjvdrdocdtlweekly(character varying, character varying, character varying, character varying)

-- DROP PROCEDURE IF EXISTS dwh.usp_f_cbadjadjvdrdocdtlweekly(character varying, character varying, character varying, character varying);

CREATE OR REPLACE PROCEDURE dwh.usp_f_cbadjadjvdrdocdtlweekly(
...
--Updating etlactiveind for Deleted source data 
		...
		UPDATE	dwh.F_cbadjadjvdrdocdtl t1
		SET		etlactiveind		=  0,
				etlupdatedatetime	= Now()::TIMESTAMP
		FROM	dwh.F_cbadjadjvdrdocdtl t
		LEFT JOIN stg.stg_cbadj_adjv_drdoc_dtl s
        ON		s.ou_id 		= t.ou_id
    	AND 	s.adj_voucher_no = t.adj_voucher_no
    	AND 	s.dr_doc_ou 	= t.dr_doc_ou
    	AND 	s.dr_doc_type 	= t.dr_doc_type
    	...
    	AND 	s.term_no 		= t.term_no
    	AND 	s.voucher_tran_type		= t.voucher_tran_type
		WHERE	t.adj_vdr_doc_dtl_key	= t1.adj_vdr_doc_dtl_key
...
END;
$BODY$;
ALTER PROCEDURE dwh.usp_f_cbadjadjvdrdocdtlweekly(character varying, character varying, character varying, character varying)
    OWNER TO proconnect;
