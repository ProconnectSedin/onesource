-- PROCEDURE: dwh.usp_f_cdiinvoicehdrweekly(character varying, character varying, character varying, character varying)

-- DROP PROCEDURE IF EXISTS dwh.usp_f_cdiinvoicehdrweekly(character varying, character varying, character varying, character varying);

CREATE OR REPLACE PROCEDURE dwh.usp_f_cdiinvoicehdrweekly(
...
/*	LEFT JOIN dwh.d_financebook f
  ...
  GET DIAGNOSTICS inscnt = ROW_COUNT;
    --Updating etlactiveind for Deleted source data 
        ...
        UPDATE  dwh.F_cdiinvoicehdr t1
        SET     etlactiveind        =  0,
                etlupdatedatetime   = Now()::TIMESTAMP
        FROM    dwh.F_cdiinvoicehdr t
        LEFT JOIN stg.stg_cdi_invoice_hdr s
        ...
        AND t.lgt_invoice = s.lgt_invoice
        AND t.MAIL_SENT = s.MAIL_SENT
        WHERE   t.cdi_inv_hdr_key       = t1.cdi_inv_hdr_key
...
END;
$BODY$;
ALTER PROCEDURE dwh.usp_f_cdiinvoicehdrweekly(character varying, character varying, character varying, character varying)
    OWNER TO proconnect;
