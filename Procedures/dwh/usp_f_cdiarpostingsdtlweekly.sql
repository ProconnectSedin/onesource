-- PROCEDURE: dwh.usp_f_cdiarpostingsdtlweekly(character varying, character varying, character varying, character varying)

-- DROP PROCEDURE IF EXISTS dwh.usp_f_cdiarpostingsdtlweekly(character varying, character varying, character varying, character varying);

CREATE OR REPLACE PROCEDURE dwh.usp_f_cdiarpostingsdtlweekly(
...
--Updating etlactiveind for Deleted source data 
		...
		UPDATE	dwh.f_cdiarpostingsdtl t1
		SET		etlactiveind		=  0,
				etlupdatedatetime	= Now()::TIMESTAMP
		FROM	dwh.f_cdiarpostingsdtl t
		LEFT JOIN stg.stg_cdi_ar_postings_dtl s
		ON	t.tran_type	 		= s.tran_type
		AND	t.tran_ou	 		= s.tran_ou
		AND	t.tran_no	 		= s.tran_no
		AND t.posting_line_no	= s.posting_line_no
		AND t.ctimestamp	 	= s.ctimestamp
		WHERE	t.cdi_dtl_key		= t1.cdi_dtl_key
...
END;
$BODY$;
ALTER PROCEDURE dwh.usp_f_cdiarpostingsdtlweekly(character varying, character varying, character varying, character varying)
    OWNER TO proconnect;
