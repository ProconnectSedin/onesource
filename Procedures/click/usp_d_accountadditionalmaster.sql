CREATE OR REPLACE PROCEDURE click.usp_d_accountadditionalmaster()
    LANGUAGE plpgsql
AS
$$
...
BEGIN
    ...
    INSERT INTO click.d_accountadditionalmaster(acc_mst_key, company_code, fb_id, usage_id, effective_from,
                                                currency_code, drcr_flag, dest_fbid, child_company, dest_company,
                                                sequence_no, timestamp, account_code, effective_to, resou_id,
                                                usage_type, ard_type, flag, createdby, createddate, modifiedby,
                                                modifieddate, etlactiveind, etljobname, envsourcecd, datasourcecd,
                                                etlcreatedatetime)
    SELECT s.acc_mst_key,
           s.company_code,
           s.fb_id,
           s.usage_id,
           s.effective_from,
