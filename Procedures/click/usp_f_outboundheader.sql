...
--UPDATE FOR OUB_ORDERQUALIFIEDDATE AND OUB_ORDERQUALIFIEDDATEKEY STARTS;	

	UPDATE CLICK.F_OUTBOUNDHEADER A
	SET oub_orderqualifieddate=CASE WHEN COALESCE(A.OUB_MODIFIED_DATE,A.OUB_CREATED_DATE)::TIME>=B.CUTOFFTIME
								THEN(((COALESCE(A.OUB_MODIFIED_DATE,A.OUB_CREATED_DATE)) + INTERVAL '1 DAY')::Date ||' '||B.OPENINGTIME)::TIMESTAMP
								ELSE COALESCE(A.OUB_MODIFIED_DATE,A.OUB_CREATED_DATE) END,
		OUB_CUTOFFTIME   =B.CUTOFFTIME,
		OUB_OPENINGTIME  =B.OPENINGTIME
	FROM CLICK.F_OUTBOUNDHEADER A1 
	LEFT JOIN DWH.D_OUTBOUNDLOCSHIFTDETAIL B
	ON  A1.OUB_OU	= B.OU
	AND A1.OUB_LOC_CODE = B.LOCATIONCODE
	AND A1.OUB_ORDER_TYPE = B.ORDERTYPE
	AND A1.OUB_SHIPMENT_TYPE = B.SERVICETYPE
	WHERE A.OBH_HR_KEY= A1.OBH_HR_KEY
	AND COALESCE(A.etlupdatedatetime, A.etlcreatedatetime)>= V_MAXDATE;


	UPDATE CLICK.F_OUTBOUNDHEADER A
	SET oub_orderqualifieddate =	CASE WHEN A1.oub_orderqualifieddate::DATE = HOLIDAYDATE AND A1.OUB_OPENINGTIME IS NOT NULL
									THEN NEXTWORKINGDATE + (A1.OUB_OPENINGTIME || ' MINUTES')::INTERVAL
		 WHEN A1.oub_orderqualifieddate::DATE = HOLIDAYDATE AND A1.OUB_OPENINGTIME IS NULL
									THEN NEXTWORKINGDATE + ('09:30:00' || ' MINUTES')::INTERVAL
									ELSE A1.oub_orderqualifieddate END		
	FROM CLICK.F_OUTBOUNDHEADER A1 
	LEFT JOIN CLICK.F_LOCATIONHOLIDAY
	ON A1.OUB_LOC_CODE = LOCATIONCODE
	AND A1.oub_orderqualifieddate::DATE = HOLIDAYDATE
	WHERE A.OBH_HR_KEY=A1.OBH_HR_KEY
	AND COALESCE(A.etlupdatedatetime, A.etlcreatedatetime)>= V_MAXDATE;


	UPDATE CLICK.F_OUTBOUNDHEADER A
	SET oub_orderqualifieddatekey = D.DATEKEY
	FROM CLICK.D_DATE D	
	WHERE DATEACTUAL = oub_orderqualifieddate::DATE
	AND COALESCE(A.etlupdatedatetime, A.etlcreatedatetime)>= V_MAXDATE;

--UPDATE FOR OUB_ORDERQUALIFIEDDATE AND OUB_ORDERQUALIFIEDDATEKEY ENDS;	
