-- Table: dwh.f_sdintcddtl

-- DROP TABLE IF EXISTS dwh.f_sdintcddtl;

CREATE TABLE IF NOT EXISTS dwh.f_sdintcddtl
(
    sdintcddtl_key bigint NOT NULL GENERATED ALWAYS AS IDENTITY ( INCREMENT 1 START 1 MINVALUE 1 MAXVALUE 9223372036854775807 CACHE 1 ),
    ...
    tcd_level character varying(10) COLLATE public.nocase,
    tcd_rate numeric(13,2),
    taxable_amount numeric(13,2),
    ...
    CONSTRAINT f_sdintcddtl_pkey PRIMARY KEY (sdintcddtl_key),
    CONSTRAINT f_sdintcddtl_ukey UNIQUE (tran_type, tran_ou, tran_no, line_no, item_tcd_code, item_tcd_var, tcd_version),
    CONSTRAINT f_sdintcddtl_currency_key_fkey FOREIGN KEY (currency_key)
        REFERENCES dwh.d_currency (curr_key) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT f_sdintcddtl_itm_hdr_key_fkey FOREIGN KEY (itm_hdr_key)
...
ALTER TABLE IF EXISTS dwh.f_sdintcddtl
    OWNER to proconnect;
-- Index: f_sdintcddtl_key_idx1

-- DROP INDEX IF EXISTS dwh.f_sdintcddtl_key_idx1;

CREATE INDEX IF NOT EXISTS f_sdintcddtl_key_idx1
    ON dwh.f_sdintcddtl USING btree
